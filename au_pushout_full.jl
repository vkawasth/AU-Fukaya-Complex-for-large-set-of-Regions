...
function padic_gate(c1::Symbol, c2::Symbol, T12_data)
    ...
    # Both contribute only if T12 carries the pole
    T12_pole = T12_data !== nothing &&
               :sAMY ∈ Set(T12_data.regions)
    v_comp   = T12_pole ? v1 + v2 : min(v1, v2)
    gate     = abs(v_comp)
    trigger  = gate > 0 ? 5^gate : 0
    return gate, trigger, v1, v2, v_comp
end
...
function resolve_pushout(c1::Symbol, c2::Symbol)
    ...
    # Step 5: p-adic gate
    gate, trigger, v1, v2, v_comp = padic_gate(c1, c2, T12)
