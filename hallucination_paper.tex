...
\begin{abstract}
Standard machine learning operates on real-valued continuous embeddings
and probability measures that are invariant under smooth perturbations.
Hallucinations, however, correspond to \emph{discrete structural
obstructions} only detectable in quotient, cohomological, and
finite-field representations --- not in $\mathbb{R}$-based metrics alone.

The core thesis:
\begin{quote}
\emph{Continuous real-valued representations do not preserve homological
invariants of the underlying discrete semantic graph.
Hallucinations, defined as nontrivial homology classes in a chain
complex of semantic continuations, are not detectable in
$\mathbb{R}$-embedding space without additional quotient or
cohomological structure.}
\end{quote}

We replace $\mathbb{R}$-valued decoding with a multiscale algebraic
criterion over coefficient fields $\mathbb{Z}_2$ and $\mathbb{Z}_5$,
inducing homological invariants on a finite chain complex $(C_*, d)$
with explicitly verified $d^2 = 0$:
\[
...
\frac{P_\theta(x_{t+1} \mid X_{\leq t})
  \cdot \prod_{n=1}^{5}\mathbf{1}_{\{ob_n(x_{t+1}\mid X_{\leq t})=0\}}}
     {Z}
\]
$\mathbb{Z}_2$ detects structural presence (orientation-collapsed);
$\mathbb{Z}_5$ detects cyclic closure obstructions ($H_1$ exactly);
2-adic valuation measures stability under refinement.
...
(one filled 2-cell, one hole; anti-FSM result via shared edges),
validate the Postnikov filtration empirically on a protocol
state-machine benchmark, and apply the framework to the 81-station
Hong Kong MTR transit network.
\end{abstract}
...
The experiments support a three-level semantic model:
$H_1(\mathbb{Z}_5)$ classifies global trajectory admissibility;
$\mathbb{Z}_2$ removes orientation sensitivity (a different view of
the same topology, not a weaker detector);
and $p$-adic valuations describe prefix stability during generation,
forming a clean separation between
\emph{topology} (what the complex is),
\emph{representation} (how it is viewed), and
\emph{dynamics} (how the model traverses it).
