...
function build_contexts(stops::Set)
    ...
    ctx[:CTX_ALL] = build_nno_au(:CTX_ALL, "Q_7P full",
        ...
        :A, 89, 62, 1.2599; initial_node=:CA1sp)
    # coker=62: confirmed value for sAMY↔Infra boundary (dp_core.jl §3)
    ctx
end
...
# =============================================================================
# PART 9: GAME COUNTERFACTUAL EXPLORER
# =============================================================================
#
# At each decision point in the game, save a GameCheckpoint.
# From any checkpoint, fork into multiple branches to compare:
#   - What actually happened (the policy's choice)
#   - What greedy would have done
#   - What look-ahead would have done
#   - Any user-specified alternative
#
# This answers: "at decision point R2, if norcain had blocked LA→sAMY
# instead of CA1sp→sAMY, what would P_max have been at round R5?"

"""
A lightweight game checkpoint: stop set + P_max + stratum + round number.
Cheap to store — no NNO context copy needed since we rebuild contexts
from stops on restore (build_contexts is fast).
"""
struct GameCheckpoint
    ...
    p_max       ::Float64
    stratum     ::Int
    plucker     ::Vector{Float64}   # Gr(3,5) Plücker at this point
    description ::String
end

"""Save a game checkpoint at the current state."""
function game_checkpoint(round::Int, stops::Set, p_max::Float64,
                          description::String="")::GameCheckpoint
    ctx     = build_contexts(stops)[:CTX_ALL]
    pl      = compute_analytic_plucker(ctx)
    stratum = stratum_from_pmax(p_max)
    GameCheckpoint(round, copy(stops), p_max, stratum,
                   Float64.(pl), description)
end

"""
    game_branch(cp, extra_stops; label) -> (p_max, trajectory)

From a GameCheckpoint, apply additional stops and simulate forward
for n_rounds using the look-ahead policy.
Returns the P_max trajectory and final state.

This is the counterfactual: "what if, at this point, we had
also blocked edge e?"
"""
function game_branch(cp       ::GameCheckpoint,
                      extra_stops::Vector{Tuple{Symbol,Symbol}},
                      n_rounds ::Int = 10;
                      label    ::String = "branch",
                      verbose  ::Bool = false)

    current_stops = union(cp.stops, Set(extra_stops))
    p_max         = run_opiate(current_stops)
    bl            = blockable_edges(current_stops)
    traj          = Float64[p_max]
    actions       = Union{Tuple{Symbol,Symbol},Nothing}[nothing]

    verbose && @printf("  [%s] Start: P_max=%.4f\n", label, p_max)
    ...
    for rnd in 1:n_rounds
        ...
        push!(traj, p_max)
        push!(actions, action)
        verbose && @printf("  [%s] R%d: P_max=%.4f  %s\n",
            label, rnd, p_max,
            action === nothing ? "—" : "$(action[1])→$(action[2])")
    end

    return traj, actions, current_stops
end

"""
    explore_counterfactuals(checkpoints; n_rounds, verbose)

Given a list of GameCheckpoints saved during a game run,
explore what would have happened under alternative choices at each
key decision point.

For each checkpoint where the actual policy took action A:
  - Branch 1: actual (A already applied)
  - Branch 2: greedy alternative (highest Renkin-Crone weight)
  - Branch 3: no action (what if norcain did nothing?)
  - Branch 4+: any user-specified alternatives

Prints a comparison table showing how P_max evolves under each branch.
"""
function explore_counterfactuals(checkpoints ::Vector{GameCheckpoint};
                                  n_rounds    ::Int = 10,
                                  verbose     ::Bool = true)

    verbose && begin
        println("\n" * "="^68)
        println("COUNTERFACTUAL EXPLORER")
        println("="^68)
    end
    ...
    for (ci, cp) in enumerate(checkpoints)
        # Only explore checkpoints where a real decision was made
        # i.e. where the P_max at this round is actionable
        cp.p_max <= NASH_FLOOR + 0.005 && continue

        bl_here  = blockable_edges(cp.stops)
        available = [(e,w) for (e,w) in bl_here if e ∉ cp.stops]
        isempty(available) && continue

        # Actual policy action (already encoded in next checkpoint)
        # Greedy: highest Renkin-Crone weight incoming to sAMY
        greedy_e = nothing
        for (e,_) in bl_here
            e ∉ cp.stops && e[2] == :sAMY && (greedy_e = e; break)
        end

        # Look-ahead / AU-QKV action
        ranked   = try au_rank_edges(cp.stops, cp.p_max)
                   catch; available[1:1]; end
        lookah_e = isempty(ranked) ? nothing : ranked[1][1]

        # Build branches
        branch_specs = [
            ("No action",  Tuple{Symbol,Symbol}[]),
            ("Greedy",     greedy_e === nothing ? [] : [greedy_e]),
            ("AU-QKV",     lookah_e === nothing ? [] : [lookah_e]),
        ]

        # Add any greedy-vs-lookah difference cases
        if greedy_e !== nothing && lookah_e !== nothing && greedy_e != lookah_e
            push!(branch_specs, ("Greedy alt ($(greedy_e[1])→$(greedy_e[2]))",
                                  [greedy_e]))
        end

        verbose && begin
            println("\n  Decision point R$(cp.round)  " *
                    "P_max=$(Base.round(cp.p_max,digits=4))  " *
                    "Stratum=$(cp.stratum)")
            println("  Current stops: $(length(cp.stops)) edges")
            println("  " * "─"^64)
            @printf("  %-22s  %-8s  %-8s  %-8s  %s\n",
                    "Branch", "Start", "After 1", "After 3", "Final")
            println("  " * "─"^64)
        end

        for (blabel, extra) in branch_specs
            traj, acts, final_stops = game_branch(cp, extra, n_rounds;
                                                   label=blabel,
                                                   verbose=false)
            a1 = length(traj) >= 2 ? traj[2] : traj[1]
            a3 = length(traj) >= 4 ? traj[4] : traj[end]
            af = traj[end]
            ...
            push!(results, (
                ...
                p_after3 = a3,
                p_final  = af,
                rounds   = length(traj) - 1,
                extra_stops = extra,
            ))

            if verbose
                marker = af <= NASH_FLOOR + 0.01 ? " ✓" :
                         af < cp.p_max * 0.5    ? " ↓" : ""
                @printf("  %-22s  %-8.4f  %-8.4f  %-8.4f  %.4f%s\n",
                        blabel, cp.p_max, a1, a3, af, marker)
            end
        end
        verbose && println("  " * "─"^64)
    end

    verbose && println("\n  ✓ = Nash floor reached   ↓ = >50%% reduction")
    verbose && println("="^68)
    return results
end

"""
    run_game_with_checkpoints(n_rounds, stops; verbose)

Run the two-agent game and save a checkpoint at every round.
Returns (history, checkpoints) so counterfactuals can be explored.
"""
function run_game_with_checkpoints(n_rounds::Int = 20;
                                    stops   ::Set  = BASE_STOPS,
                                    verbose ::Bool  = true)
    current_stops = copy(stops)
    bl            = blockable_edges(stops)
    ...
    verbose && begin
        println("="^68)
        println("TWO-AGENT GAME WITH CHECKPOINTS")
        println("="^68)
        @printf("  %-5s %-10s %-8s  %-22s  %-8s\n",
                "Round","P_max","Stratum","Action","Δ P_max")
        println("─"^68)
    end

    for rnd in 1:n_rounds
        # Save checkpoint BEFORE action (this is the decision point)
        cp = game_checkpoint(rnd, current_stops, p_max,
                              "R$(rnd) decision point")
        push!(checkpoints, cp)
...
# =============================================================================
# PART 10: CONTEXT INSPECTOR
# =============================================================================
# Unified extraction layer for NNOAUContext.
# Every field the context carries can be read, formatted, or exported.

"""
    ContextSnapshot

All extractable information from one NNOAUContext at a point in time.
"""
struct ContextSnapshot
    ...
    n_stops     ::Int
    # Probability state
    prob        ::Vector{Float64}       # current NNO probs as Float64
    prob_nno    ::Vector{NNOProb}       # exact NNO
    prob_sum    ::Float64               # should be 1.0
    top_nodes   ::Vector{Pair{Symbol,Float64}}  # sorted by prob
    # Markov structure
    trans_mat   ::Matrix{Float64}       # T as Float64
    stationary  ::Vector{Float64}       # power-iteration stationary dist
    mixing_time ::Int                   # steps until max|p - π| < 0.01
    # Pharmacodynamic Plücker
    plucker     ::Vector{Float64}       # [p12,p13,p14,p23,p24,p34]
    stratum     ::Int                   # Schubert stratum 0-4
    sector      ::Symbol                # :A/:B/:C/:D/:generic
    # Algebraic invariants
    hh2         ::Int                   # HH²(W_α)
    coker       ::Int                   # coker(ρ*_αβ)
    rho         ::Float64               # ρ(B_Λ) spectral radius
    klein_err   ::Float64               # |p12p34 - p13p24 + p14p23|
    # Fisher metric
    r_eff       ::Dict{Tuple{Symbol,Symbol},Float64}
    fisher_risk ::Float64               # aggregate risk ∈ (0,1]
    # Stop architecture
    stops       ::Set{Tuple{Symbol,Symbol}}
    active_edges::Vector{Tuple{Symbol,Symbol}}
    # Edge weights (top 5 by Renkin-Crone weight)
    top_edges   ::Vector{Tuple{Tuple{Symbol,Symbol},Float64}}
end

"""
    inspect(ctx) -> ContextSnapshot

Extract all available information from an NNOAUContext.
"""
function inspect(ctx::NNOAUContext)::ContextSnapshot
    n       = length(ctx.regions)
    T_f64   = Float64.(ctx.trans_mat)
    prob_f  = Float64.(ctx.prob)

    # ── Stationary distribution via power iteration ──────────────────────
    pi_vec  = fill(1.0/max(n,1), n)
    for _ in 1:200
        pi_new = T_f64 * pi_vec
    ...
    # ── Mixing time ───────────────────────────────────────────────────────
    p_test  = zeros(n); p_test[1] = 1.0
    mix_t   = 0
    for k in 1:1500
        p_test = T_f64 * p_test
        s = sum(p_test); s > 0 && (p_test ./= s)
        maximum(abs.(p_test .- pi_vec)) < 0.01 && (mix_t = k; break)
    end
    mix_t == 0 && (mix_t = -1)   # -1 = not converged in 1500 steps

    # ── Top nodes by probability ──────────────────────────────────────────
    top_nodes = sort([ctx.regions[i] => prob_f[i] for i in 1:n],
                     by=x->x.second, rev=true)[1:min(5,n)]

    # ── Plücker and stratum ───────────────────────────────────────────────
    pl         = compute_analytic_plucker(ctx)
    # P_max via DP (not max(prob) which is just the δ_CA1sp init state)
    dp_pmax    = run_opiate(ctx.stops)
    stratum    = stratum_from_pmax(dp_pmax)
    # Klein quadric residual
    p12,p13,p14,p23,p24,p34 = pl
    klein = abs(p12*p34 - p13*p24 + p14*p23)

    # ── Fisher metric ─────────────────────────────────────────────────────
    r_eff_table, risk = fisher_metric_projection(Set(ctx.stops))

    # ── Top edges by weight ───────────────────────────────────────────────
    edge_weights = [(e, Float64(get(ctx.weights, e, NNO_ONE)))
                    for e in ctx.edges]
    top_edges = sort(edge_weights, by=x->x[2], rev=true)[1:min(5,end)]

    ContextSnapshot(
        ctx.id, ctx.label, ctx.regions, n,
        length(ctx.edges), length(ctx.stops),
        prob_f, ctx.prob, sum(prob_f), top_nodes,
        T_f64, pi_vec, mix_t,
        Float64.(pl), stratum, ctx.sector,
        ctx.hh2, ctx.coker, ctx.rho, klein,
        r_eff_table, risk,
        ctx.stops, ctx.edges,
        top_edges)
end
...
"""
    print_snapshot(snap; verbose=false)

Print a formatted summary of a ContextSnapshot.
"""
function print_snapshot(snap::ContextSnapshot; verbose::Bool=false)
    println("┌─ Context: $(snap.id) — $(snap.label)")
    println("│  Regions: $(snap.n_regions)   " *
            "Active edges: $(snap.n_edges)   " *
            "Stops: $(snap.n_stops)")

    # Probability state
    @printf("│  Prob sum: %.6f  (should be 1.0)\n", snap.prob_sum)
    println("│  Top nodes by probability:")
    for (node, p) in snap.top_nodes
        bar = "█"^Int(floor(p*20)) * "░"^(20-Int(floor(p*20)))
        @printf("│    %-10s %s %.4f\n", node, bar, p)
    end

    # Markov structure
    mix_str = snap.mixing_time < 0 ?
               ">1500 (sAMY↔HPF oscillator, τ≈830)" :
               "$(snap.mixing_time) steps"
    @printf("│  Mixing time: %s to ε<0.01\n", mix_str)
    println("│  Stationary distribution (top 3):")
    top_stat = sort(collect(enumerate(snap.stationary)), by=x->x[2], rev=true)
    for (i,p) in top_stat[1:min(3,end)]
        @printf("│    %-10s %.4f\n", snap.regions[i], p)
    end

    # Plücker / Schubert
    sect = Dict(4=>"A (open cell)", 3=>"B (transition)",
                2=>"C (crisis)", 1=>"D (near-base)", 0=>"D (surgery)")
    @printf("│  Plücker: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f]\n",
            snap.plucker...)
    @printf("│  Stratum: %d  Sector: %s\n",
            snap.stratum, get(sect, snap.stratum, "?"))
    @printf("│  Klein residual: %.2e  (%s)\n",
            snap.klein_err,
            snap.klein_err < 1e-6 ? "✓ valid Gr(2,4)" : "⚠ not on Klein quadric")

    # Algebraic invariants
    @printf("│  HH²: %d   coker: %d   ρ(B_Λ): %.4f\n",
            snap.hh2, snap.coker, snap.rho)
    @printf("│  coker=62: %s\n",
            snap.coker == 62 ? "YES — no stable Nash equilibrium" :
            snap.coker == 0  ? "no — stable NE exists" : "partial obstruction")

    # Fisher metric
    @printf("│  Fisher risk: %.4f  %s\n",
            snap.fisher_risk,
            "█"^Int(floor(snap.fisher_risk*20)) *
            "░"^(20-Int(floor(snap.fisher_risk*20))))
    println("│  Top flow edges (lowest R_eff):")
    sorted_r = sort(collect(snap.r_eff), by=x->x[2])
    for (e,r) in sorted_r[1:min(3,end)]
        w = Float64(get(GAME_WEIGHTS, e, NNO_ONE))
        @printf("│    %-10s→%-10s  R_eff=%.4f  w=%.1f\n",
                e[1], e[2], r, w)
    ...
    # Top edges
    if verbose
        println("│  Top active edges by weight:")
        for (e,w) in snap.top_edges
            @printf("│    %-10s→%-10s  w=%.1f\n", e[1], e[2], w)
        end
        println("│  Active stops:")
        for s in sort(collect(snap.stops), by=string)
            @printf("│    %-10s→%-10s\n", s[1], s[2])
        end
    end
    println("└─────────────────────────────────────────────────────")
...
"""
    compare_snapshots(snaps, labels)

Side-by-side comparison of multiple ContextSnapshots.
Useful for comparing baseline vs post-block vs counterfactual.
"""
function compare_snapshots(snaps ::Vector{ContextSnapshot},
                            labels::Vector{String})
    println("\n" * "="^68)
    println("CONTEXT COMPARISON")
    println("="^68)
    @printf("  %-20s  %s\n", "Metric",
            join([lpad(l, 12) for l in labels], "  "))
    println("  " * "─"^(20 + 14*length(snaps)))

    metrics = [
        ("P_max (DP)", s -> begin
            p = run_opiate(s.stops)
            @sprintf("%.4f", p)
        end),
        ("P(sAMY) init",s -> begin
            i = findfirst(==(:sAMY), s.regions)
            i === nothing ? "n/a" : @sprintf("%.4f", s.prob[i])
        end),
        ("Stratum",    s -> string(s.stratum)),
        ("Mixing T",   s -> s.mixing_time < 0 ? ">1500" : string(s.mixing_time)),
        ("p12",        s -> @sprintf("%.3f", s.plucker[1])),
        ("p34",        s -> @sprintf("%.3f", s.plucker[6])),
        ("Klein err",  s -> @sprintf("%.1e", s.klein_err)),
        ("Fisher risk",s -> @sprintf("%.4f", s.fisher_risk)),
        ("HH²",        s -> string(s.hh2)),
        ("coker",      s -> string(s.coker)),
        ("ρ(B_Λ)",     s -> @sprintf("%.4f", s.rho)),
        ("n_stops",    s -> string(s.n_stops)),
        ("n_edges",    s -> string(s.n_edges)),
    ]

    for (name, fn) in metrics
        vals = [fn(s) for s in snaps]
        @printf("  %-20s  %s\n", name,
                join([lpad(v, 12) for v in vals], "  "))
    end
    println("="^68)
end

"""
    inspect_game_checkpoint(cp) -> ContextSnapshot

Extract a full context snapshot from a GameCheckpoint.
"""
function inspect_game_checkpoint(cp::GameCheckpoint)::ContextSnapshot
    ctx = build_contexts(cp.stops)[:CTX_ALL]
    return inspect(ctx)
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for (stop_label, stop_set) in [
        ...
        # ── Context inspector: baseline snapshot ─────────────────────────
        println("\n[0] CONTEXT SNAPSHOT — Baseline")
        snap_baseline = inspect(build_contexts(stop_set)[:CTX_ALL])
        print_snapshot(snap_baseline; verbose=false)
        ...
        # ── Two-agent game with checkpoints ─────────────────────────────
        println("\n[B] TWO-AGENT GAME — Look-ahead policy (with checkpoints)")
        history_2, checkpoints = run_game_with_checkpoints(20;
                                     stops=stop_set, verbose=true)

        # ── Counterfactual explorer ───────────────────────────────────────
        println("\n[B2] COUNTERFACTUAL EXPLORER")
        println("  At each decision point: what if norcain had chosen differently?")
        explore_counterfactuals(checkpoints; n_rounds=8, verbose=true)

        # ── Context comparison: baseline vs post-game ─────────────────────
        if length(checkpoints) >= 2
            println("\n[B3] CONTEXT COMPARISON: Baseline vs Final")
            snap_final = inspect_game_checkpoint(checkpoints[end])
            compare_snapshots([snap_baseline, snap_final],
                              ["Baseline", "Post-game"])
        end
